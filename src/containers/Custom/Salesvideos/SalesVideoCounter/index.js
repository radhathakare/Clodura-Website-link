...
const SalesVideoCounter = ({ page, setPage, totalPages }) => {
    const renderNumberBlocks = () => {
        const renderedBlock = [];
        for (let i = 0; i < totalPages; i++)
            renderedBlock.push(<div style={{ backgroundColor: page === (i + 1) ? '#03BAED' : 'white', 
            color: page === (i + 1) ? 'white' : 'gray' }} onClick={() => setPage((i + 1))}>{i + 1}</div>)
...
export default SalesVideoCounter;